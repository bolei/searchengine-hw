...
public class TermVector {
	...
	public TermVector(int docId, String fieldName) throws IOException {
		 ...
		 */
		this.luceneTerms = MiscUtil.getIndexReader().getTermVector(docId,
				fieldName);
		 ...
		 * information, and finding the position of the last term. The 0'th term
		 * indicates a stopword, so this loop starts at i=1.
		 */
		...
		for (int i = 1; ithTerm.next() != null; i++) {
			...
			/**
			 * Find the position of the last (indexed) term in the document, so
			 * that the positions array can be created and populated later. The
			 * last position for each term is the largest, so ignore the
			 * positions before it.
			 */
		...
		/**
		 * Create and fill the positions array. Note that the stems array uses
		 * stem 0 to indicate a stopword, so "real" stems have indexs 1 through
		 * length+1.
		 */
	...
	/**
	 * The frequency of the n'th stem in the current doc, or -1 if the index is
	 * invalid. The frequency for stopwords (i=0) is not stored (0 is returned).
	 */
	...
	public long totalStemFreq(int i) throws IOException {
		return MiscUtil.getIndexReader().totalTermFreq(terms[i]);
	}
	...
	public int stemDf(int i) throws IOException {
		return MiscUtil.getIndexReader().docFreq(terms[i]);
	}
